static char rcsid[]="$Id: f_malloc8.c 289 2015-01-29 00:14:38Z user $";
/*
...
HOST_FTN_PTR
HOST_FTN_FUNC(fdds_malloc8, FDDS_MALLOC8)(
   HOST_FTN_DBL *size8)
{
   HOST_FTN_DBL tmp8;
   size_t size;
   ...
   tmp8 = size;
   if (tmp8 != *size8) {
      ddsPanic("(fdds_malloc8) system cannot handle this size (%lg != %lg)\n",
         *size8, tmp8);
   }

   return cdds_malloc(size);
}
