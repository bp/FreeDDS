static char rcsid[]="$Id: c_scanv.c 16 2009-01-06 21:52:47Z user $";
/*
   ...
   const char **value)
{
   ...
   if(dds_scan_dict) {
      ...
      if(dds_scan_token.next == dds_scan_dict->buf.ptr +
         dds_scan_newer->start) {
         status = EOF;
      } else {
         status = dds_scan_dict->buf.ptr + dds_scan_newer->start
            - dds_scan_token.next;
         *value = dds_scan_token.next;
         dds_scan_token.next = dds_scan_dict->buf.ptr +
            dds_scan_newer->start;
      }
   } else {
      status = EOF;
