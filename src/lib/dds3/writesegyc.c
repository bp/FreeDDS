static char rcsid[]="$Id: writesegyc.c 219 2011-02-16 01:11:40Z user $";
/*
RCS-t=DDS API, write segy card header binary data
   ...
   int count)
{
   ...
   if (count != 1) {
      ddsError("Can only write 1 segy card preamble, not count=%d\n\tbinary= %s\n",
               count, bin->name);
      return DDS_BAD;
   }

   memcpy(bin->std.segy.card_buf, buffer, type->stroke);
   ...
   if (!(bin->state & (DDS_BIN_STATE)(DDS_BIN_FREEZE | DDS_BIN_EOF | DDS_BIN_EOM)) &&
      ...
      /* point back to beginning of the Card Header */
      if(gsetpos(media->fp, &media->data_bias)) {
         ddsError("seek setpos, %s\n\t%s, card header offset= %lu\n",
                  strerror(errno), media->title.ptr,
      ...
      if (n_bytes != type->stroke) {
         ddsWarn("Only wrote %d segy card header bytes of %d\n",
                 n_bytes, type->stroke);
      }
