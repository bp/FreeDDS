static char rcsid[]="$Id: c_putcb.c 16 2009-01-06 21:52:47Z user $";
/*
RCS-t=DDS API, put char, blank filled
   ...
   if(sym->type) {
      ...
      if(prec) {
         ...
         if(nbytes && status == DDS_CON_OK) {
            CON_AASS(type->ptype, DDS_CTYPE_CHAR, &pad, " ");
            memset((char*)buf + num * prec, pad, nbytes);
         } else {
            num = 0;
         }
      } else {
         ddsDebug("cdds_putcb: invalid type\n\tbin=%s\n\tsymbol= %s, type= %s\n",
            bin->name, sym->name ? sym->name : "<anonymous>",
            ddsHintType(bin, type));
         num = 0;
      }
   } else {
      ddsDebug("cdds_putcb: type not defined\n\tbin=%s\n\tsymbol= %s\n",
         bin->name, sym->name ? sym->name : "<anonymous>");
      num = 0;
   }
   DDS_API_EXIT(num);
