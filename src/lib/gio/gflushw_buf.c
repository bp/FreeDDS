static char rcsid[]="$Id: gflushw_buf.c 13 2009-01-06 16:56:43Z user $";
/*
FLUSH WRITE (buffered)
...
int GIO_gflushw_buf(  /* return 0 (error == EOF) */
   GFILE *fp)           /* GIO stream */
{
   ...
   if(n_want) {
      ...
      while(n_want > 0) {
         if(gio.trc_file) {
            ...
            fprintf(gio.trc_file, "WRITE: flushw_buf fd=%d nybtes=%d time(sec)=%g\n", 
