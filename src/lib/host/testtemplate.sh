...
EXE=`ddsarg arg=exe      default=none $*`
INPUTDIR=`ddsarg arg=inputdir default=none $* | sed 's/ //g'`
TESTDIR=`ddsarg arg=testdir  default=none $*`
if [ "$EXE" = "none" -o "$INPUTDIR" = "none" -o "$TESTDIR" = "none" ]; then
...
#
#INPUTDIR=$INPUTDIR/MY_TEST_DATA
